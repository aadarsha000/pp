...
from .models import Interview, InterviewStatus, FeedbackRubric, InterviewFeedback, FeedbackScore
...
class InterviewSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        for interviewer in interviewers:
            conflict_qs = Interview.objects.filter(
            ...
            for existing in conflict_qs:
                ...
                if existing.scheduled_at < end_time and existing_end > scheduled_at:
                    conflicting_interview = existing
                    break
            if conflicting_interview:
                raise InterviewConflictException(
                    detail={
                        "interviewer": interviewer.username,
                        "conflicting_interview_id": conflicting_interview.id,
                        "conflicting_start": conflicting_interview.scheduled_at,
                        "conflicting_end": conflicting_interview.end_time,
                    }
                )
        return data
...
class InterviewFeedbackSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if InterviewFeedback.objects.filter(interview=interview, interviewer=interviewer).exists():
            raise serializers.ValidationError("You have already submitted feedback for this interview.")
            
        if not interview.interviewers.filter(id=interviewer.id).exists():
            raise serializers.ValidationError("Only assigned interviewers can submit feedback.")

        scores = data.get('scores', [])
        if not scores:
            raise serializers.ValidationError({"scores": "At least one score is required."})
        rubric_ids = [item['rubric'].id for item in scores]
        if len(rubric_ids) != len(set(rubric_ids)):
            raise serializers.ValidationError({"scores": "Duplicate rubric entries are not allowed."})
